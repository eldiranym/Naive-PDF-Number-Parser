        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for i, page in enumerate(pdf.pages):
                ...
                if page_context_multiplier != Decimal('1'):
                    print(f"  - Page {page_num}: Page-level context multiplier found ({page_context_multiplier:,})")
                ...
                for t_idx, table in enumerate(tables):
                    ...
                    for word in page.extract_words():
                        ...
                        if bbox[0] <= word["x0"] <= bbox[2]:  
                            if abs(word["top"] - bbox[1]) < 80 or abs(word["bottom"] - bbox[3]) < 80:
                                nearby_text += " " + word["text"]
                    nearby_match = CONTEXT_MULTIPLIER_RE.search(nearby_text)
                    if nearby_match:
                        multiplier_word = next((g for g in nearby_match.groups() if g is not None), None)
                        if multiplier_word and multiplier_word.lower() in MULTIPLIERS:
                            table_context_multiplier = MULTIPLIERS[multiplier_word.lower()]
                            print(f"  - Page {page_num}: Table-level context multiplier found near table -> '{multiplier_word}' ({table_context_multiplier:,})")
                    ...
                    for col_idx, header_cell in enumerate(header_row):
                        ...
                        for word, multiplier in MULTIPLIERS.items():
                            if re.search(r'\b' + re.escape(word) + r'\b', header_text_lower):
                    ...
                    for row in table[1:]:
                        ...
                        is_excluded_row = any(kw in row[0].lower() for kw in ROW_EXCLUSION_KEYWORDS)
                        ...
                        for col_idx, cell in enumerate(row):
                            ...
                            for match in matches:
                                ...
                                context_mult = column_multipliers.get(col_idx,
                                                table_context_multiplier if not is_excluded_row
                                                else Decimal('1'))
                                value = parse_value(num_str, multiplier_word, context_mult)

                                if value > max_value:
                                    max_value = value
                                    max_page = page_num
                                    print(f"  - Page {page_num} (Table): New max value found -> {max_value:,.2f} (from cell: '{cell.strip()}')")

                # --- Step 3: Process Non-Table Text ---
                all_words = page.extract_words()
                non_table_words = [word for word in all_words if not is_within_bboxes(word, table_bboxes)]
                ...
                if non_table_text:
                    ...
                    for match in matches:
                        ...
                        if value > max_value:
                            max_value = value
                            max_page = page_num
                            print(f"  - Page {page_num} (Non-Table): New max value found -> {max_value:,.2f} (from text: '{match.group(0).strip()}')")
